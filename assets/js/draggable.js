...
/**
 * EvalQuiz Class
 * Quiz d'auto-évaluation interactif en Vanilla JavaScript (sans Vue.js)
 */
class EvalQuiz {
  static counter = 0;
  ...
  constructor(element, options = {}) {
    ...
    this.id = this.container.id || `eval_quiz_${EvalQuiz.counter}`;
    this.container.id = this.id;

    this.title = options.title || this.container.getAttribute('data-title') || 'Quiz Diagnostique (4 Questions)';
    this.questions = options.questions || this.parseQuestions();
  ...
  parseQuestions() {
    const qNodes = Array.from(this.container.querySelectorAll('.quiz-question, [data-correct]'));
    if (qNodes.length > 0) {
      return qNodes.map((qNode, idx) => {
        const textNode = qNode.querySelector('.question-text, .question, h4, h5, h6, p');
        const questionText = textNode ? textNode.textContent.trim() : (qNode.getAttribute('data-question') || `Question ${idx + 1}`);

        const optNodes = Array.from(qNode.querySelectorAll('.quiz-options li, .quiz-option, .option'));
        const options = optNodes.map(opt => opt.textContent.trim());

        const correctAttr = qNode.getAttribute('data-correct');
        const correct = correctAttr !== null ? parseInt(correctAttr, 10) : 0;

        const explNode = qNode.querySelector('.explanation, .quiz-explanation');
        const explanation = explNode ? explNode.textContent.trim() : (qNode.getAttribute('data-explanation') || '');
    ...
    const dataAttr = this.container.getAttribute('data-questions');
    if (dataAttr) {
  ...
  getScore() {
    ...
    this.questions.forEach((q, idx) => {
      if (this.answers[idx] !== undefined && this.answers[idx] === q.correct) {
        score++;
      }
    });
    return score;
  ...
  render() {
    ...
    let html = `
      <div class="card card-custom">
        <div class="card-header-custom d-flex justify-content-between align-items-center">
          <h3 class="h5 mb-0"><i class="bi bi-patch-question-fill text-warning me-2"></i> ${this.escapeHtml(this.title)}</h3>
          <span class="badge badge-info-custom">Score : ${score} / ${total}</span>
        </div>
        <div class="card-body">
    `;
    ...
    this.questions.forEach((q, qIdx) => {
      ...
      html += `
        <div class="mb-4 pb-3 border-bottom border-secondary border-opacity-25 quiz-question-block" data-qindex="${qIdx}">
          <h6 class="mb-3">
            <span class="badge bg-secondary me-2">Q${qIdx + 1}</span> ${this.escapeHtml(q.question)}
          </h6>
      ...
      q.options.forEach((optText, oIdx) => {
        ...
        let iconClass = 'bi-circle me-2';

        if (isAnswered) {
          if (oIdx === q.correct) {
            optionClass += ' selected-correct';
            iconClass = 'bi-check-circle-fill text-success me-2';
          } else if (userAnswer === oIdx) {
            optionClass += ' selected-incorrect';
            iconClass = 'bi-x-circle-fill text-danger me-2';
          }
        }

        html += `
          <div class="${optionClass}" data-qindex="${qIdx}" data-oindex="${oIdx}" style="${isAnswered ? 'pointer-events: none;' : 'cursor: pointer;'}">
            <i class="bi ${iconClass}"></i> ${this.escapeHtml(optText)}
          </div>
        `;
      });
      ...
      if (isAnswered && q.explanation) {
        html += `
          <div class="mt-2 small text-muted fst-italic section-active-fade">
            <i class="bi bi-info-circle text-info me-1"></i> ${this.escapeHtml(q.explanation)}
          </div>
        `;
      }
    ...
    html += `
          <div class="text-center mt-3">
            <button class="btn btn-outline-warning btn-reset-quiz">Réinitialiser le Quiz</button>
          </div>
        </div>
      </div>
    `;
  ...
  attachEvents() {
    ...
    options.forEach(opt => {
      opt.addEventListener('click', (e) => {
        e.preventDefault();
        const qIdx = parseInt(opt.getAttribute('data-qindex'), 10);
        const oIdx = parseInt(opt.getAttribute('data-oindex'), 10);

        if (this.answers[qIdx] !== undefined) return;

        this.answers[qIdx] = oIdx;
        this.render();

        if (typeof window.showToast === 'function') {
          const isCorrect = (oIdx === this.questions[qIdx].correct);
          if (isCorrect) {
            window.showToast("Bonne réponse !", "bi-check-circle-fill text-success");
          } else {
            window.showToast("Mauvaise réponse", "bi-x-circle-fill text-danger");
          }
        }
      });
    });

    const resetBtn = this.container.querySelector('.btn-reset-quiz');
    if (resetBtn) {
      resetBtn.addEventListener('click', (e) => {
        e.preventDefault();
        this.reset();
        if (typeof window.showToast === 'function') {
          window.showToast("Quiz réinitialisé !");
        }
      });
    }
  }
...
function initAllDraggablesAndQuizzes() {
  ...
  document.querySelectorAll('.eval-quiz, .eval-quiz-container')
    .forEach(item => {
      if (!item.dataset.evalQuizInit) {
        item.dataset.evalQuizInit = 'true';
        new EvalQuiz(item);
      }
    });
}
...
if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', initAllDraggablesAndQuizzes);
} else {
  initAllDraggablesAndQuizzes();
}
