...
* En algorithmique, le passage par référence s'indique avec `@` (`@pf1`).
* En Python :
  ```python
  def Nom_module (pf1, pf2, ..., pfn) :
      Traitement
  ```
> **N.B. :** En Python, les paramètres de type **dictionnaire**, **tableau (`numpy`)** et **fichier** sont, par défaut, **passés par référence**.
